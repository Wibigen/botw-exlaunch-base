...
MODULES := sead NintendoSDK xlink2 aal agl EventFlow gsys hkStubs

MODULE_INCLUDES := $(foreach mod,$(MODULES),-I$(PWD)/lib/$(mod)/include)

EXTRA_INCLUDES := -I$(PWD)/lib -I$(PWD)/src

C_FLAGS   := $(filter-out -Wno-invalid-offsetof,$(C_FLAGS))
CXX_FLAGS += -Wno-invalid-offsetof

# Built internal C flags variable
EXL_CFLAGS   := $(C_FLAGS) -DNNSDK=1 $(MODULE_INCLUDES) $(EXTRA_INCLUDES) -Wno-deprecated-declarations -DEXL_LOAD_KIND=$(LOAD_KIND) -DEXL_LOAD_KIND_ENUM=$(LOAD_KIND_ENUM) -DEXL_PROGRAM_ID=0x$(PROGRAM_ID)

# Built internal C++ flags variable
EXL_CXXFLAGS := $(CXX_FLAGS) -DNNSDK=1 $(MODULE_INCLUDES) $(EXTRA_INCLUDES) -Wno-deprecated-declarations
